...
#. Restore the Bootloader
   ...
   * If the PI5 still cannot boot, the bootloader may be corrupted. You can follow this guide: :ref:`update_bootloader_promax` and choose whether to boot from SD card or NVMe/USB.
...
4. Confirm that your NVMe SSD has a properly installed operating system. Refer to: :ref:`install_the_os_promax`.

5. If the wiring is correct and the OS is installed, but the NVMe SSD still fails to boot, try booting from a Micro SD card to verify the functionality of other components. Once confirmed, proceed to: :ref:`configure_boot_ssd_promax`.
...
Once you have completed the installation of the ``pironman5`` module, you will be able to access the :ref:`promax_view_control_dashboard`.
