...
The onboard dual 3.3V power supply supports a maximum output of 3A (10W). Both power rails operate independently without interference.

**FORCE ENABLE**  
The onboard power supply is activated by the switch signal from the PCIe interface. After the Raspberry Pi boots up, the signal turns on the 3.3V power. If the system does not support the switch signal or due to other reasons, short the J4 FORCE EN jumper to forcibly enable the onboard 3.3V power for NVMe.
