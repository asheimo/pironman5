...
19. Pironman5 Max の振動スイッチ機能を使用できますか？
----------------------------------------------------------------------------------------------------------------------------

Pironman5 Max の振動スイッチ機能は、バージョン 1.3.6 以降ではソフトウェア的に互換性がなくなりました。これはバージョンアップによる変更であり、ハードウェアの故障ではありません。この機能に依存している場合は、以前のバージョンへのロールバック、手動によるコードの修正、または公式サポートへの要請をご検討ください。OLED 画面の表示時間については、引き続きコマンドラインから調整可能です。OLED 画面はボタンを一度押すことで起動し、数秒間表示が持続します。


20. ダッシュボードにデータが表示されません（温度、統計、グラフが空です）
----------------------------------------------------------------------------------------------------------------------------

1. ブラウザのキャッシュをクリアするか、ブラウザのシークレットモード/プライベートモードでダッシュボードページにアクセスしてみてください。

2. 現在の状態をさらに確認するには、まず以下の2つのサービスが正常に動作しているかどうかを確認することをお勧めします。
   ...
   両方のサービスで ``active (running)`` と表示されるはずです。

3. サービスが正常に動作していない場合は、以下を実行してみてください。
   ...
   その後、約30秒待ってからダッシュボードページを更新し、状態を確認してください。
