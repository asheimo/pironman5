# Pironman 5 — Hardware Highlights

## Chassis & Dimensions
- **Size**: 111.9 × 78.5 × 117 mm
- **Material**: Silver anodized aluminum alloy main body + transparent acrylic side panels
- **Weight**: Premium build with spring-loaded microSD slot (tool-free access)
- **Support Platform**: Raspberry Pi 5 only

## Power
- USB-C 5 V/5 A input
- 27 W official or SunFounder 27 W PD supply **strongly recommended**, especially when an NVMe drive is populated
...
## Storage & Expansion — Single NVMe
- **Direct PCIe Gen2 x1 lane** from the Raspberry Pi 5 (no PCIe switch).
- **One M.2 M-key slot**: supports 2230 / 2242 / 2260 / 2280 sizes.
- Use cases:
  - NVMe SSD for fast storage or boot drive
  - Hailo-8/8L AI accelerator for edge AI workloads
- **Bandwidth**: ~500 MB/s (direct from Pi 5's PCIe Gen2 x1 lane).
- **PCIe Gen3 not officially supported**, but may work with forced config.
- FFC/FPC cable connects the PCIe breakout to the Pi 5.
...
- **Optional modules**: Camera, 3.5" screen, RTL-SDR.
...
- **Dual NVMe + AI accelerator simultaneously** — only one M.2 slot available.

## Comparison with Other Pironman 5 Variants

| Feature | Pironman 5 | Max | Mini | Pro Max |
|---------|------------|-----|------|---------|
| NVMe Slots | 1 | 2 (switch) | 1 | 2 (switch) |
| PCIe Switch | No | ASM1182e | No | Yes |
| OLED | 0.96" | 0.96" | None | 0.96" |
| Touchscreen | No | No | No | 4.3" |
| RGB LEDs | 4× WS2812 | 4× WS2812 | 4× WS2812 | 6× WS2812B |
| Chassis Color | Silver | Dark | Silver | Dark |
| GPIO Fans | 2 | 2 + 2 fan LEDs | 1 RGB fan | 3 RGB PWM |
| Speakers | No | No | No | Stereo 3W |
| HDMI | Full-size | Full-size | Micro | Full-size |
| Dimensions (mm) | 111.9×78.5×117 | 111.9×78.5×117 | 67.8×98.6×101.5 | 140.9×77.0×138.7 |

## Quick Specs Summary
| Parameter | Value |
|-----------|-------|
| Dimensions | 111.9×78.5×117 mm |
| Material | Silver aluminum alloy + transparent acrylic |
| Platform | Raspberry Pi 5 |
| Power | USB-C 5V/5A (27W PD recommended) |
| OLED | 0.96" 128×64 I2C |
| RGB LEDs | 4× WS2812-5050 + expansion via RGB OUT |
| NVMe Slots | 1× M.2 M-key (2230-2280), PCIe Gen2 |
