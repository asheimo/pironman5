...
- **PiPower5 UPS (optional)**: Install with `--pipower5` flag for battery backup support. After installation, use `pironman5 pipower5` subcommand for UPS control (battery status, shutdown percentage, buzzer config, email alerts). OLED pages include `battery`, `input`, `rpi_power` when UPS is active. See ``pipower5 --help`` for full UPS CLI.
